...
export class OfferComponent extends BaseComponent implements OnInit, AfterViewInit {
	...
	constructor(
		...
		this.model = Utils.getModel('Offer');
	}
	...
	evaluateButtons() {
		...
		this.buttons.push({
			...
			title: 'Tedarikçi Teklifleri Raporu',
			icon: 'cloud_download',
	...
	getReport() {
		const dialogRef = this.dialog.open(ReportDialogComponent, { data: { filter: 'date', title: 'Tedarikçi Teklifleri Raporu' } });
		dialogRef.afterClosed().subscribe(res => {
			if (res) {
				...
				this.http.post('api/' + this.model.apiName + '/report?startDate=' + res.startDate + '&endDate=' + res.endDate, undefined, { headers: httpHeaders, responseType: 'blob' })
					.pipe(
						tap(res2 => {
							Utils.downloadFile(res2, 'Excel', 'Tedarikçi Teklifleri Raporu');
