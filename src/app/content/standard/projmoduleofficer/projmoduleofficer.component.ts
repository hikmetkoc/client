...
@Component({
	selector: 'kt-projmoduleofficer',
	templateUrl: './projmoduleofficer.component.html',
	changeDetection: ChangeDetectionStrategy.Default,
	styleUrls: ['./projmoduleofficer.component.scss'],
	encapsulation: ViewEncapsulation.None
})
export class ProjModuleOfficerComponent extends BaseComponent implements OnInit, AfterViewInit {
	...
	constructor(
		...
		this.model = Utils.getModel('ProjModuleOfficer');
	}
	...
	rowClicked(row, reload?) {
		if (reload) {
			...
			this.defaultFilter = [{
				name: 'projmoduleofficer.id',
				operator: 'EQUALS',
				value: row.id
			}];
			this.defaultValues = [{
				field: 'projmoduleofficerId',
				value: row.id
			}, {
				field: 'projmoduleofficer',
				value: row
			}];
			this.current = row;
		}
