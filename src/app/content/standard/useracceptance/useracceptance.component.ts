...
} from "../../_base/detail/reportmanager/report-manager-dialog/report-manager-dialog.component";
import {
	IkReportManagerDialogComponent
} from "../../_base/detail/reportmanager/ikreport-manager-dialog/ikreport-manager-dialog.component";

@Component({
	selector: 'kt-useracceptance',
	templateUrl: './useracceptance.component.html',
	changeDetection: ChangeDetectionStrategy.Default,
	styleUrls: ['./useracceptance.component.scss'],
	encapsulation: ViewEncapsulation.None
})
export class UserAcceptanceComponent extends BaseComponent implements OnInit, AfterViewInit {
	...
	constructor(
		...
		this.model = Utils.getModel('UserAcceptance');
	...
	rowClicked(row, reload?) {
		if (reload) {
			...
			this.defaultFilter = [{
				name: 'useracceptance.id',
				operator: 'EQUALS',
				value: row.id
			}];
			this.defaultValues = [{
				field: 'useracceptanceId',
				value: row.id
			}, {
				field: 'useracceptance',
				value: row
			}];
			this.current = row;
		}
	...
	/*showDocumentModal(sendDocumentModal) {
		if (this.documents.length === 0) {
			this.baseService.find(undefined, 'ikfiles/documents').subscribe((res) => {
	...
	sendDocuments() {
		...
		console.log(docsToSend + ' ---- ' + this.current.id);
		if (docsToSend.length > 0) {
			this.baseService.loadingSubject.next(true);
			this.baseService.update({ ikfileId: this.current.id, docs: docsToSend }, 'ikfiles/documents').subscribe(() => {
	...
	/*getReport() {
		const dialogRef = this.dialog.open(ReportDialogComponent, { data: { filter: 'date', title: 'İşlem Raporu' } });
		dialogRef.afterClosed().subscribe(res => {
			if (res) {
				...
				this.http.post('api/' + this.model.apiName + '/report?startDate=' + res.startDate + '&endDate=' + res.endDate, undefined, { headers: httpHeaders, responseType: 'blob' })
					.pipe(
						tap(res2 => {
							Utils.downloadFile(res2, 'Excel', 'İslemler Raporu');
							this.baseService.loadingSubject.next(false);
						}),
						catchError(err => {
							this.baseService.loadingSubject.next(false);
	...
	showFiles() {
		const dialogRef = this.dialog.open(IkReportManagerDialogComponent, {
			width: '800px',
