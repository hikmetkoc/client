...
@Component({
	selector: 'kt-projmodulesprint',
	templateUrl: './projmodulesprint.component.html',
	changeDetection: ChangeDetectionStrategy.Default,
	styleUrls: ['./projmodulesprint.component.scss'],
	encapsulation: ViewEncapsulation.None
})
export class ProjModuleSprintComponent extends BaseComponent implements OnInit, AfterViewInit {
	...
	constructor(
		...
		this.model = Utils.getModel('ProjModuleSprint');
	}
	...
	rowClicked(row, reload?) {
		if (reload) {
			...
			this.defaultFilter = [{
				name: 'projmodulesprint.id',
				operator: 'EQUALS',
				value: row.id
			}];
			this.defaultValues = [{
				field: 'projmodulesprintId',
				value: row.id
			}, {
				field: 'projmodulesprint',
				value: row
			}];
			this.current = row;
		}
