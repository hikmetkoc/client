...
export class BuyComponent extends BaseComponent implements OnInit, AfterViewInit {
	...
	getReport() {
		const dialogRef = this.dialog.open(ReportDialogComponent, { data: { filter: 'date', title: 'Teklifler Raporu' } });
		dialogRef.afterClosed().subscribe(res => {
			if (res) {
				...
				this.http.post('api/' + this.model.apiName + '/report?startDate=' + res.startDate + '&endDate=' + res.endDate, undefined, { headers: httpHeaders, responseType: 'blob' })
					.pipe(
						tap(res2 => {
							Utils.downloadFile(res2, 'Excel', 'Teklifler Raporu');
