...
@Component({
	selector: 'kt-projtaskactivity',
	templateUrl: './projtaskactivity.component.html',
	changeDetection: ChangeDetectionStrategy.Default,
	styleUrls: ['./projtaskactivity.component.scss'],
	encapsulation: ViewEncapsulation.None
})
export class ProjTaskActivityComponent extends BaseComponent implements OnInit, AfterViewInit {
	...
	constructor(
		...
		this.model = Utils.getModel('ProjTaskActivity');
	}
	...
	rowClicked(row, reload?) {
		if (reload) {
			...
			this.defaultFilter = [{
				name: 'projtaskactivity.id',
				operator: 'EQUALS',
				value: row.id
			}];
			this.defaultValues = [{
				field: 'projtaskactivityId',
				value: row.id
			}, {
				field: 'projtaskactivity',
				value: row
			}];
			this.current = row;
		}
