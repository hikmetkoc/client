...
@Component({
	selector: 'kt-projtaskofficer',
	templateUrl: './projtaskofficer.component.html',
	changeDetection: ChangeDetectionStrategy.Default,
	styleUrls: ['./projtaskofficer.component.scss'],
	encapsulation: ViewEncapsulation.None
})
export class ProjTaskOfficerComponent extends BaseComponent implements OnInit, AfterViewInit {
	...
	constructor(
		...
		this.model = Utils.getModel('ProjTaskOfficer');
	}
	...
	rowClicked(row, reload?) {
		if (reload) {
			...
			this.defaultFilter = [{
				name: 'projtaskofficer.id',
				operator: 'EQUALS',
				value: row.id
			}];
			this.defaultValues = [{
				field: 'projtaskofficerId',
				value: row.id
			}, {
				field: 'projtaskofficer',
				value: row
			}];
			this.current = row;
		}
