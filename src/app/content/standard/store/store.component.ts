...
export class StoreComponent extends BaseComponent implements OnInit, AfterViewInit {
	...
	constructor(
		...
		this.model = Utils.getModel('Store');
	}
	...
	getReport() {
		const dialogRef = this.dialog.open(ReportDialogComponent, { data: { filter: 'date', title: 'Satın Alma Raporu' } });
		dialogRef.afterClosed().subscribe(res => {
			if (res) {
				...
				this.http.post('api/' + this.model.apiName + '/report?startDate=' + res.startDate + '&endDate=' + res.endDate, undefined, { headers: httpHeaders, responseType: 'blob' })
					.pipe(
						tap(res2 => {
							Utils.downloadFile(res2, 'Excel', 'Satın Alma Raporu');
	...
	talepidBul(id: number) {
		...
		this.baseService.find(queryParams, 'stores').subscribe(res => {
			for (const tid of res.body) {
				if (tid.buyowner.id !== this.baseService.getUser().id) { continue; }
