...
@Component({
	...
	templateUrl: './user-trial-form-dialog.component.html',
	styleUrls: ['./user-trial-form-dialog.component.scss'],
	encapsulation: ViewEncapsulation.None,
})
export class UserTrialFormDialogComponent implements OnInit {
	@Input() current: any;
	@Input() model: any;
	chiefData = [];
	managerData = [];
...
\tcolor: black;\
	...
	getData() {
		...
		this.baseService.find(queryParams, 'user_trial_forms').subscribe(res => {
			this.chiefData = res.body.filter(flt => flt.authorization.id === 'Form_Auth_Chief');
			this.managerData = res.body.filter(flt => flt.authorization.id === 'Form_Auth_Manager');
			this.ikData = res.body.filter(flt => flt.authorization.id === 'Form_Auth_Ik');
