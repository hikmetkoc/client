...
export class Utils {
	...
	static ibanMask: Array<RegExp | string> = ['T', 'R', /\d/, /\d/, ' ', /\d/, /\d/, /\d/, /\d/, ' ', /\d/, /\d/, /\d/, /\d/, ' ', /\d/, /\d/, /\d/, /\d/, ' ', /\d/, /\d/];
	...
	public static ibanClean(iban) {
		...
		if (iban.length > 26) { iban = iban.split('-')[0]; }
		iban = iban.replace(/\D+/g, '');
		if (iban.length > 26) { iban = iban.substr(0, 26); } else { iban = iban.substr(0, 24); }
		return iban;
	}
	...
	public static ibanFormat(iban) {
		...
		iban = this.ibanClean(iban);
		if (iban.length === 26) {
			iban = 'TR' + iban.substring(2, 4) + ' ' + iban.substring(4, 8) + ' ' + iban.substring(8, 12) + ' ' + iban.substring(12, 16) + ' ' + iban.substring(16, 20) + ' ' + iban.substring(20, 22);
		} else {
			iban = 'TR' + iban.substring(2, 4) + ' ' + iban.substring(4, 8) + ' ' + iban.substring(8, 12) + ' ' + iban.substring(12, 16) + ' ' + iban.substring(16, 20);
		}
		return iban;
	}
